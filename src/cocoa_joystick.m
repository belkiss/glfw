...
void _glfwInitJoysticks(void)
{
    ...
    while ((ioHIDDeviceObject = IOIteratorNext(objectIterator)))
    {
        ...
        // Check device type
        valueRef = CFDictionaryGetValue(hidProperties, CFSTR(kIOHIDPrimaryUsagePageKey));
        if (valueRef)
        {
        ...
        valueRef = CFDictionaryGetValue(hidProperties, CFSTR(kIOHIDPrimaryUsageKey));
        if (valueRef)
        {
        ...
        valueRef = CFDictionaryGetValue(hidProperties, CFSTR(kIOHIDProductKey));
        if (valueRef)
        {
